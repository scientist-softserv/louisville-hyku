  ...
  module Actors
    ...
    class FileSetActor
      ...
      def create_content(file, relation = :original_file, from_url: false)
        ...
        Sidekiq.logger.error("FileSetActor.create_content is starting #{Time.now.utc} :: file_set.title #{file_set.title}") # rubocop: disable Metrics/LineLength
        if from_url
        ...
        Sidekiq.logger.error("FileSetActor.create_content is ending #{Time.now.utc} :: file_set.title #{file_set.title}") # rubocop: disable Metrics/LineLength
      end
      ...
      def attach_to_work(work, file_set_params = {})
        ...
        Sidekiq.logger.error("FileSetActor.attach_to_work is ending #{Time.now.utc} :: work.id #{work.id}") # rubocop: disable Metrics/LineLength
      end
